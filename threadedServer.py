            ...
            if auth == 1:
                ...
                for i in range(len(usernames)):
                    if user == usernames[i] and psswd == passwords[i]:
                        ...
                        logs = 1
                        x = 1
                        clientList[k] = (clientList[k][0],clientList[k][1],x,clientList[k][3])
    ...
    try :
        while True:
            ...
            for i in range(len(clientList)) :
                if a == clientList[i][0]:
                    temp = 1
                    timeClient = clientList[i][3]
                    timeCurr = time.time()
                    #print type(addr[1])
                    if int(clientList[i][1]) < limitRequest :
                         x = 0
                         if timeCurr - timeClient < timeLimit :
                             timeCurr = timeClient
                             x = clientList[i][1] + 1
                         clientList[i] = (clientList[i][0],x,clientList[i][2],timeCurr)
                    else :
                         if timeCurr - timeClient < timeLimit + blockTime :
                             clientList[i] = (clientList[i][0],0,clientList[i][2],time.time())
                             pass
            ...
            if temp == 0 :
                auth = 1
                clientList.append((a,0,0,time.time()))
            #print clientList,count
            if count >= limitRequest :
                print "Over Requesting! Attack detected! Buckle Up!"
                continue
